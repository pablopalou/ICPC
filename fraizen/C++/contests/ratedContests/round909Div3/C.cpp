...
void solve() {
    ...
    //chequeo si son todos negativos y ahi respondo el maximo de los negativos!
    bool negativos = true;
    ll maximo = -1e9;
    fr(0,n){
    ...
    //caso normal de positivos y negativos
    ll cur = a[0];
    ll res = max(ll(0), cur);
    fr(1,n){
        if (((a[i] & 1) && ((a[i-1] & 1) == 0)) || ((a[i] & 1) == 0 && (a[i-1] & 1))){ //paridad distinta del anterior
            if (cur > 0){
                cur += a[i];
                res = max(cur, res);
            } else {
        ...
        } else { // no se puede, arranco en 0 o agarro este.
            cur = max(ll(0), a[i]);
            res = max(res, a[i]);
        }
    }
